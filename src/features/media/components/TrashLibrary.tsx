...
import { ArrowClockwise, TrashSimple, Warning, ShieldWarning, Download, Hash, Broom, Folder as FolderIcon, FolderSimple } from "@phosphor-icons/react";
...
export default function TrashLibrary({ initialItems, folders = [] }: { initialItems: MediaItem[]; folders?: Folder[] }) {
 ...
 const [isProcessing, setIsProcessing] = useState<string | null>(null);
  ...
  setIsProcessing(id);
  const result = await moveMediaToFolderAction([id], folderId);
  if (!result.success) {
  toast.error(result.error || "Move failed");
  setIsProcessing(null);
  return;
  }
  const restored = await restoreFromTrashAction(id);
  if (!restored.success) {
  toast.error(restored.error || "Restore failed");
  setIsProcessing(null);
  return;
  }
  toast.success(folderId ? "Restored to folder" : "Restored to library");
  router.refresh();
  setIsProcessing(null);
  };
...
const menuItems = [
  ...
  { label: "Move to Folder", icon: FolderSimple, onClick: () => {}, divider: true, subItems: [
  { label: "Library Root", icon: FolderIcon, onClick: () => handleMoveToFolder(item.id, null) },
...
<ContextMenu items={menuItems}>
  <MediaCard item={item} hideContextMenu />
  </ContextMenu>
