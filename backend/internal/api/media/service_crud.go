...
func (s *Service) List(ctx context.Context, userID string, folderID *string, favorites, trash, vault, dedup bool, search string, page, limit int) (*ListResponse, error) {
	...
	if dedup {
		...
		items, total, err = scanMediaPageWithTotal(ctx, tdb, query, totalCol, f.args)
	} else {
...
func (s *Service) Get(ctx context.Context, userID, id string) (*MediaItem, error) {
	tdb, err := s.pool.Get(ctx, userID)
	if err != nil {
		return nil, fmt.Errorf("get tenant db: %w", err)
	}
...
// mediaExecer is the shared Exec surface of *db.TenantDB and *sql.Tx, so the
// media insert can run either directly or inside the quota transaction.
// (Both embed the database/sql ExecContext.)
...
func insertMedia(ctx context.Context, e mediaExecer, id, userID, title, filePath, mimeType string, size int64, width, height *int, hash string, folderID *string, capturedAt *int64, metadata *string, duration *int, transcodeStatus *string, now int64) (bool, error) {
...
func (s *Service) Create(ctx context.Context, userID, folderID, filePath, title, mimeType, hash string, size int64, width, height *int, capturedAt *int64, metadata *string, duration *int, transcodeStatus *string) (*MediaItem, bool, error) {
	tdb, err := s.pool.Get(ctx, userID)
	if err != nil {
	...
	inserted, err := insertMedia(ctx, tdb, id, userID, title, filePath, mimeType, size, width, height, hash, fID, capturedAt, metadata, duration, transcodeStatus, now)
	if err != nil {
...
func (s *Service) CreateWithinQuota(ctx context.Context, userID, folderID, filePath, title, mimeType, hash string, size int64, width, height *int, capturedAt *int64, metadata *string, duration *int, transcodeStatus *string) (*MediaItem, bool, error) {
	tdb, err := s.pool.Get(ctx, userID)
	if err != nil {
		return nil, false, fmt.Errorf("get tenant db: %w", err)
	}
	...
	inserted, err := insertMedia(ctx, tx, id, userID, title, filePath, mimeType, size, width, height, hash, fID, capturedAt, metadata, duration, transcodeStatus, now)
	if err != nil {
		return nil, false, err
	}
	if !inserted {
...
func (s *Service) SaveEditorOverwrite(ctx context.Context, userID, mediaID, filePath, hash string, width, height int, size int64, mimeType, metadata string) error {
	tdb, err := s.pool.Get(ctx, userID)
	if err != nil {
...
func (s *Service) Update(ctx context.Context, userID, id string, updates map[string]interface{}) error {
	tdb, err := s.pool.Get(ctx, userID)
	if err != nil {
		return fmt.Errorf("get tenant db: %w", err)
	}
...
func (s *Service) Delete(ctx context.Context, userID, id string) (*MediaItem, error) {
	...
	res, err := tdb.Exec(ctx, "DELETE FROM media WHERE user_id = $1 AND id = $2", userID, id)
	if err != nil {
...
func (s *Service) HashExists(ctx context.Context, userID, hash string) (bool, error) {
	tdb, err := s.pool.Get(ctx, userID)
	if err != nil {
	...
	defer tdb.Close()
	var existing string
	err = tdb.QueryRow(ctx, "SELECT id FROM media WHERE user_id = $1 AND hash = $2 LIMIT 1", userID, hash).Scan(&existing)
	if err == sql.ErrNoRows {
		return false, nil
	}
	if err != nil {
...
func (s *Service) UpdateByHash(ctx context.Context, userID, hash string, updates map[string]interface{}) error {
	tdb, err := s.pool.Get(ctx, userID)
	if err != nil {
		return fmt.Errorf("get tenant db: %w", err)
	}
	...
	var id string
	err = tdb.QueryRow(ctx, "SELECT id FROM media WHERE user_id = $1 AND hash = $2", userID, hash).Scan(&id)
	if err == sql.ErrNoRows {
