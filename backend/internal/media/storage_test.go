...
func TestServeFile_SymlinkInsideMediaDir(t *testing.T) {
	...
	// Media files are encrypted on disk, so write a valid encrypted JPEG-like
	// payload rather than a raw plaintext one.
	jpeg := []byte{0xFF, 0xD8, 0xFF, 0xE0, 0x00, 0x10, 'J', 'F', 'I', 'F', 'x', 'y', 'z'}
	var enc bytes.Buffer
	if err := s.masterKey.EncryptToFile(&enc, bytes.NewReader(jpeg)); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(real, enc.Bytes(), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if !bytes.Equal(rec.Body.Bytes(), jpeg) {
		t.Fatal("served bytes differ from stored plaintext")
	}
}
...
func newTestStorage(dir string) *Storage {
	return NewStorage(dir, testMasterKey())
}

func TestStorage_EncryptAndServeRoundTrip(t *testing.T) {
	s := newTestStorage(t.TempDir())
	userID := "testuser"

	plain := []byte{0xFF, 0xD8, 0xFF, 0xE0, 0x00, 0x10, 'J', 'F', 'I', 'F', 0xDE, 0xAD, 0xBE, 0xEF}
	filename, mediaPath, _, err := s.SaveFileFromReader(userID, bytes.NewReader(plain), "abc.jpg")
	if err != nil {
		t.Fatalf("save: %v", err)
	}
	if filename != "abc.jpg" {
		t.Fatalf("filename = %s, want abc.jpg", filename)
	}

	// On-disk file must be the encrypted envelope, not the plaintext.
	onDisk, err := os.ReadFile(mediaPath)
	if err != nil {
		t.Fatal(err)
	}
	if !IsEncrypted(onDisk) {
		t.Fatal("stored media file is not encrypted (missing PRE1 header)")
	}

	// ServeFile decrypts and returns the original bytes.
	c, rec := newEchoContext()
	if err := s.ServeFile(c, userID, "abc.jpg"); err != nil {
	...
	if err != nil {
		t.Fatalf("open decrypted temp: %v", err)
	}
	tmpName := decrypted.Name()
	got, err := io.ReadAll(decrypted)
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(got, plain) {
		t.Fatal("decrypted temp bytes differ from original")
	}
	if err := decrypted.Close(); err != nil {
		t.Fatalf("close decrypted temp: %v", err)
	}
