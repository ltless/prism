...
import { startCardDrag } from "../utils/dragGhost";
...
function buildMenuItems(item: MediaItem, folders: FolderType[], actions: CardActions) {
  return [
    ...
    { label: "Relocate to Trash", icon: Trash, onClick: () => actions.handleDelete(), variant: "danger" as const }
  ];
}
...
export const MediaCard = memo(function MediaCard({
 ...
 const handleMouseLeave = () => { setIsHovered(false); setIsTapped(false); };
  ...
  const menuItems = buildMenuItems(item, folders, {
    isFav,
    setRenameModalOpen,
    handleMoveToFolder,
  ...
  const handleDragStart = (e: React.DragEvent) => {
    ...
    startCardDrag(e, idsToMove, imageUrl, selectedThumbs);
  };
