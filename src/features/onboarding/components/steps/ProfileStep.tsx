...
export function ProfileStep({ profileImage, coverImage, isUploading, onFileUpload }: ProfileStepProps) {
 ...
 <input type="file" className="hidden" ref={profileInputRef} onChange={(e) => onFileUpload(e, 'profile')} accept="image/*" />
  ...
  {coverImage ? (
  <Image src={`/api/media/${coverImage}`} alt="Cover" fill className="object-cover" unoptimized priority />
  ) : (
  ...
  {profileImage ? (
  <Image src={`/api/media/${profileImage}`} alt="Profile" fill className="object-cover" unoptimized priority />
  ) : (
