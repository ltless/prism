...
import { getVaultMediaAction, lockVaultAction } from "../services/mediaVaultActions";
...
export default function VaultLibraryClient({
  ...
  // F1: revokes the server-side unlock token too, so auto-lock is a real
  // lock, not just a hidden UI. Fire-and-forget; the next PIN entry re-mints.
  const lockVault = useCallback(() => {
    setIsUnlocked(false);
    setPin("");
    setError("");
    setItems([]);
    void lockVaultAction().catch(() => {});
  }, []);
  ...
  const handlePinChange = async (newPin: string) => {
    ...
    if (newPin.length === 6) {
      ...
      try {
        ...
        // Unlock is server-side now: fetch the vault list only after the PIN
        // verification response carried the unlock cookie to the browser.
        setIsLoadingItems(true);
        const itemsRes = await getVaultMediaAction();
        if (itemsRes.success && !itemsRes.locked) {
          ...
          setError(itemsRes.error || "Failed to load vault");
          setPin("");
        }
      } catch (err) {
