...
export function useMediaSelection(
 ...
 const start = displayedItems.findIndex(i => i.id === lastSelectedId);
 ...
 }, [displayedItems, lastSelectedId]);

  const handleBulkTrash = useCallback(async () => {
  const count = selectedIds.size;
  if (count === 0 || isProcessingRef.current) return;
  const ok = await confirm(`Relocate ${count} records to the trash directory?`);
  if (!ok) return;
